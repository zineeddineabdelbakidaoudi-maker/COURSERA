...
const hardcodedTools: AutomationTool[] = [
  {
    id: 1,
    title: 'E-commerce Scraper Pro',
    price: 3200,
    category: 'Web Scrapers',
    description: 'Extract product data from major Algerian marketplaces automatically.',
  ...
  {
    id: 2,
    title: 'Real Estate Auto-Poster',
    price: 5500,
    category: 'Scripts',
    description: 'Automate your property listings across multiple social platforms.',
  ...
  {
    id: 3,
    title: 'Notion Inventory Manager',
    price: 0,
    category: 'Cloud Workflows',
    description: 'Advanced Make.com workflow to sync local sales with Notion.',
  ...
  },
  {
    ...
    description: 'Chrome extension to generate Algerian-standard tax invoices instantly.',
  ...
  },
  {
    ...
    price: 4500,
    category: 'Scripts',
    description: 'Send automated updates to your clients without getting flagged.',
  ...
  {
    id: 6,
    title: 'Lead Finder AI Kit',
    price: 7200,
    category: 'Scripts',
    description: 'AI-powered script to find and verify B2B leads in the MENA region.',
...
const categories = [
  ...
  { name: 'Cloud Workflows', icon: Workflow },
  { name: 'Web Scrapers', icon: Bot },
  { name: 'Browser Extensions', icon: Cpu }
];

const otherCategories = [
  { name: 'Notion Automations', icon: Blocks },
  { name: 'API Kits', icon: FileJson },
  { name: 'Discord Bots', icon: Code2 },
  { name: 'Zapier Templates', icon: Zap },
  { name: 'System Tools', icon: Layers }
];
...
function AutomationCard({ tool }: { tool: AutomationTool }) {
  return (
      ...
      <div className="px-2 pb-2">
        <div className="mb-2 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3 text-[10px] font-medium text-gray-500">
            ...
            <span className="flex items-center gap-1"><Cpu className="h-3 w-3" /> {tool.platform}</span>
          </div>
        </div>
...
export default function AutomationPage() {
  ...
  useEffect(() => {
    async function fetchDBProducts() {
      const { data } = await supabase.from('DigitalProduct').select('*').eq('category', 'Automation').eq('status', 'live');
      if (data && data.length > 0) {
        const dbTools = data.map((d: any) => ({
          id: d.id,
          title: d.title,
          price: parseFloat(d.price) || 0,
          category: 'Scripts', // fallback
          description: d.description || 'Automation Script',
          imageUrl: d.cover_image || 'https://images.unsplash.com/photo-1551288049-bebda4e38f71?auto=format&fit=crop&q=80',
          rating: 5.0,
          downloads: 0,
          ...
          <header className="relative z-20 mb-12">
            <div className="opacity-100">
              ...
              <p className="text-lg font-light text-gray-500">
                Premium scripts, bots, and workflows to scale your digital presence.
              </p>
            </div>
          </header>
            ...
            <div className="flex items-center gap-2 overflow-x-auto pb-2 md:pb-0 scrollbar-hide">
              ...
              <DropdownMenu modal={false}>
                ...
                <DropdownMenuContent align="start" className="w-56 rounded-2xl p-2 bg-white/95 backdrop-blur-md border-gray-100 shadow-xl">
                  <DropdownMenuLabel className="text-[10px] font-bold uppercase tracking-widest text-gray-400 px-3 py-2">Advanced Tools</DropdownMenuLabel>
            ...
            <AnimatePresence mode="popLayout">
              {filteredTools.length > 0 ? (
                ...
                >
                  ...
                  <p className="text-gray-500">Try adjusting your filters or search query to find the perfect automation.</p>
                  <button 
                    onClick={() => {
