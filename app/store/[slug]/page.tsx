            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="mb-12">
                <div className="flex items-center justify-between mb-8">
                  <h2 className="text-2xl font-bold text-slate-900 dark:text-foreground">Customer Reviews</h2>
                  <Button onClick={() => setShowReviewModal(true)} variant="outline" className="gap-2">
      ...
      {showReviewModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="bg-white dark:bg-card w-full max-w-lg rounded-3xl p-6 md:p-8 shadow-2xl relative animate-in fade-in zoom-in-95">
            ...
            <p className="text-slate-500 mb-6 font-medium">Share your experience with <b>{title}</b>.</p>
            ...
            <div className="mb-6 flex justify-center gap-2">
              {[1, 2, 3, 4, 5].map(s => (
                <button key={s} onClick={() => setReviewStars(s)} className="p-1 focus:outline-none focus:scale-110 hover:scale-110 transition-transform">
                  <Star className={cn("w-10 h-10", reviewStars >= s ? "fill-amber-400 text-amber-400" : "text-slate-200 hover:text-amber-200")} />
                </button>
              ))}
            </div>

            <textarea
              className="w-full bg-slate-50 border border-slate-200 rounded-2xl p-4 min-h-[120px] mb-6 focus:outline-none focus:ring-2 focus:ring-black outline-none font-medium resize-none"
