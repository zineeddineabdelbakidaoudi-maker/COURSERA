...
const hardcodedTemplates: Template[] = [
  {
    ...
    price: 4500,
    category: 'Notion',
    description: 'Complete system to manage projects, invoices, and clients in Notion.',
  ...
  },
  {
    ...
    price: 6800,
    category: 'Figma',
    description: 'Modern and clean dashboard design system with 200+ components.',
  ...
  },
  {
    ...
    price: 3500,
    category: 'Webflow',
    description: 'Minimalist and fast-loading portfolio template for digital creators.',
  ...
  },
  {
    ...
    price: 1500,
    category: 'Canva',
    description: 'Professional presentation deck to secure investors and clients.',
  ...
  },
  {
    ...
    price: 8200,
    category: 'Figma',
    description: 'Full mobile app UI kit for shopping and marketplace platforms.',
  ...
  },
  {
    ...
    price: 0,
    category: 'Notion',
    description: 'Simple and effective content calendar for social media managers.',
...
const otherCategories = [
  ...
  { name: 'Email Templates', icon: AppWindow },
  { name: 'Presentations', icon: Presentation },
  { name: 'Icon Sets', icon: Star }
];
    ...
    <TiltCard className="group cursor-pointer rounded-[2rem] border border-gray-100 bg-white p-4 shadow-[0_8px_30px_rgb(0,0,0,0.02)] transition-all hover:shadow-[0_20px_40px_rgb(0,0,0,0.06)]">
      ...
      <div className="px-2 pb-2">
        <div className="mb-2 flex items-center justify-between">
          <span className="text-[10px] font-bold uppercase tracking-[0.1em] text-gray-400">{template.category}</span>
          <div className="flex items-center gap-3 text-[10px] font-medium text-gray-500">
            <span className="flex items-center gap-1"><ShoppingBag className="h-3 w-3" /> {template.sales}+</span>
            <span className="flex items-center gap-1"><Layout className="h-3 w-3" /> {template.platform}</span>
          </div>
        </div>
        ...
        <div className="mt-6 flex items-center justify-between border-t border-gray-50 pt-4">
          <span className="text-xl font-bold text-black">{template.price === 0 ? 'FREE' : `${template.price} DZD`}</span>
...
export default function TemplatesPage() {
  ...
  useEffect(() => {
    async function fetchDBProducts() {
      const { data } = await supabase.from('DigitalProduct').select('*').in('category', ['Template', 'Notion', 'Figma', 'Webflow']).eq('status', 'live');
      if (data && data.length > 0) {
        const dbTemplates = data.map((d: any) => ({
          id: d.id,
          title: d.title,
          price: parseFloat(d.price) || 0,
          category: d.category === 'Template' ? 'Figma' : d.category,
          description: d.description || 'Premium template',
          imageUrl: d.cover_image || 'https://images.unsplash.com/photo-1581291518633-83b4ebd1d83e?auto=format&fit=crop&q=80',
          rating: 5.0,
          sales: 0,
  ...
  const filteredTemplates = templates.filter(template => {
    const matchesCategory = selectedCategory === 'All' || template.category === selectedCategory || (otherCategories.some(c => c.name === template.category) && selectedCategory === template.category);
    const matchesSearch = template.title.toLowerCase().includes(searchQuery.toLowerCase()) || 
                          template.description.toLowerCase().includes(searchQuery.toLowerCase());
    const matchesPrice = priceRange === 'all' || 
                         (priceRange === 'under-3000' && template.price < 3000) ||
                         (priceRange === '3000-6000' && template.price >= 3000 && template.price <= 6000) ||
                         (priceRange === 'above-6000' && template.price > 6000) ||
                         (priceRange === 'free' && template.price === 0);
          ...
          <header className="relative z-20 mb-12">
            <div className="opacity-100">
              ...
              <p className="text-lg font-light text-gray-500">
                Ready-to-use digital systems, dashboards, and design assets.
              </p>
            </div>
          </header>
            ...
            <AnimatePresence mode="popLayout">
              {filteredTemplates.length > 0 ? (
                ...
                >
                  ...
                  <h3 className="mb-2 text-xl font-medium text-black">No templates found</h3>
                  <p className="text-gray-500">Try adjusting your filters or search query to find the perfect template.</p>
                  <button 
                    onClick={() => {
